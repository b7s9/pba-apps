...
class ShirtInterestDeleteView(ShirtsAreDoneMixin, LoginRequiredMixin, DeleteView):
    model = ShirtInterest
...
def create_tshirt_checkout_session(request, shirt_id):
    ...
    stripe_product_search = stripe.Product.search(
        query="active:'true' AND name:'T-Shirt Pre-Order 2024-05'"
    )
    price_search = [
        p
    ...
    if request.method == "POST":
        ...
        shirt = ShirtInterest.objects.get(id=shirt_id)
...
def complete_tshirt_checkout_session(request, shirt_id):
    ...
    if session["status"] == "complete":
        s = ShirtInterest.objects.get(id=shirt_id)
        s.paid = True
        s.save()
